...
public class Task1 {
    ...
    @SneakyThrows
    static Task1 create(String inputFile){
        ...
        byte[] s = new byte[fsize];
        try (FileInputStream  fis =
                     new FileInputStream(file)/*, "windows-1251"*/;)
        {
            fis.read(s);
        }
        return new Task1(new String(s), inputFile);
    }

    private Task1(String text, String filename){
        this.text = text;
    ...
    void writeOutputFile(){
        File file = new File("src/main/resources/"+filename+".jkwb");
        try (val out = new FileOutputStream(file);)

                //Writer out = new BufferedWriter(new OutputStreamWriter(
                //new FileOutputStream(file), "UTF8"))) {
        {
            out.write(foundKW.toString().getBytes());
        }
    }
